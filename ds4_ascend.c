...
int ds4_gpu_embed_token_hc_tensor(ds4_gpu_tensor *out_hc, const void *model_map, uint64_t model_size, uint64_t weight_offset, uint32_t n_vocab, uint32_t token, uint32_t n_embd, uint32_t n_hc) { return ascend_unimplemented(__func__); }
int ds4_gpu_embed_tokens_hc_tensor(ds4_gpu_tensor *out_hc, const ds4_gpu_tensor *tokens, const void *model_map, uint64_t model_size, uint64_t weight_offset, uint32_t n_vocab, uint32_t n_tokens, uint32_t n_embd, uint32_t n_hc) { return ascend_unimplemented(__func__); }
...
int ds4_gpu_shared_gate_up_swiglu_q8_0_tensor(ds4_gpu_tensor *gate, ds4_gpu_tensor *up, ds4_gpu_tensor *mid, const void *model_map, uint64_t model_size, uint64_t gate_offset, uint64_t up_offset, uint64_t in_dim, uint64_t out_dim, const ds4_gpu_tensor *x) {
    return ascend_matmul_q8_0_tensor(gate, model_map, model_size, gate_offset, in_dim, out_dim, x, 1) &&
           ...
           ascend_swiglu_host(mid, gate, up, out_dim, 10.0f, 1.0f);
}
...
int ds4_gpu_rope_tail_tensor(ds4_gpu_tensor *x, uint32_t n_tok, uint32_t n_head, uint32_t head_dim, uint32_t n_rot, uint32_t pos0, uint32_t n_ctx_orig, bool inverse, float freq_base, float freq_scale, float ext_factor, float attn_factor, float beta_fast, float beta_slow) {
    ...
    if (ok) ds4_ascend_launch_rope_tail_table(g_streams[x->device], x->ptr, table_dev->ptr, n_tok, n_head, head_dim, n_rot);
...
int ds4_gpu_compressor_update_tensor(const ds4_gpu_tensor *kv_cur, const ds4_gpu_tensor *sc_cur, ds4_gpu_tensor *state_kv, ds4_gpu_tensor *state_score, ds4_gpu_tensor *comp_cache, const void *model_map, uint64_t model_size, uint64_t ape_offset, uint32_t ape_type, uint64_t norm_offset, uint32_t norm_type, uint32_t head_dim, uint32_t ratio, uint32_t pos, uint32_t comp_row, uint32_t n_rot, uint32_t n_ctx_orig, float freq_base, float freq_scale, float ext_factor, float attn_factor, float beta_fast, float beta_slow, float rms_eps) { return ascend_unimplemented(__func__); }
...
int ds4_gpu_compressor_prefill_state_ratio4_tensor(ds4_gpu_tensor *state_kv, ds4_gpu_tensor *state_score, const ds4_gpu_tensor *kv_tail, const ds4_gpu_tensor *sc_tail, const void *model_map, uint64_t model_size, uint64_t ape_offset, uint32_t ape_type, uint32_t head_dim, uint32_t pos0) {
    ...
    const uint64_t state_count = (uint64_t)state_rows * width;
    if (ape_offset > model_size || (uint64_t)ratio * width * elem_ape > model_size - ape_offset || kv_tail->bytes < tail_count * sizeof(float) || sc_tail->bytes < tail_count * sizeof(float) || state_kv->bytes < state_count * sizeof(float) || state_score->bytes < state_count * sizeof(float)) return 0;
...
int ds4_gpu_add_tensor(ds4_gpu_tensor *out, const ds4_gpu_tensor *a, const ds4_gpu_tensor *b, uint32_t n) {
    ...
    if (!ascend_host_fallback_begin("add", 2u * (uint64_t)n * sizeof(float), (uint64_t)n * sizeof(float))) return 0;
    float *oh = malloc((size_t)n * sizeof(float));
    float *ah = malloc((size_t)n * sizeof(float));
    float *bh = malloc((size_t)n * sizeof(float));
    if (!oh || !ah || !bh) {
        free(oh); free(ah); free(bh);
        return 0;
    }
    int ok = ds4_gpu_tensor_read(a, 0, ah, (uint64_t)n * sizeof(float)) &&
             ds4_gpu_tensor_read(b, 0, bh, (uint64_t)n * sizeof(float));
    if (ok) {
        for (uint32_t i = 0; i < n; i++) oh[i] = ah[i] + bh[i];
        ok = ds4_gpu_tensor_write(out, 0, oh, (uint64_t)n * sizeof(float));
    }
    free(oh); free(ah); free(bh);
    return ok;
}
