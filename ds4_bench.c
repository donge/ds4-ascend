...
static void usage(FILE *fp) {
    fprintf(fp,
        ...
        "      Select backend explicitly. Defaults to Metal on macOS, Ascend on Ascend builds, CUDA elsewhere.\n"
...
static ds4_backend parse_backend(const char *s, const char *opt) {
    ...
    fprintf(stderr, "ds4-bench: invalid value for %s: %s\n", opt, s);
    fprintf(stderr, "ds4-bench: valid backends are: metal, cuda, ascend, cpu\n");
    exit(2);
}
